...
class ChamferDistance(torch.nn.Module):
    ...
    def forward(
        ...
        if bidirectional:
            if return_index:
                return chamfer_forward + chamfer_backward, forward_nn_idx, backward_nn_idx
            else:
                return chamfer_forward + chamfer_backward
        if reverse:
            if return_index:
                return chamfer_backward, backward_nn_idx
            else:
                return chamfer_backward
        if return_index:
            return chamfer_forward, forward_nn_idx
        else:
